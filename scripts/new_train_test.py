import os
import math
import random
import signal
import sys
import time
import warnings

import numpy as np
import torch
import torch.nn as nn
import torch.optim as optim
from torch.utils.data import DataLoader, random_split
from torchvision import transforms
from torchvision.transforms import Pad
from tqdm import tqdm
import matplotlib.pyplot as plt
from tabulate import tabulate
import pickle
import cv2

from EfficientLiteSeg import EfficientLiteSeg
from Dataset import TrainingDataset, TestDataset
from evaluation import dice_coefficient, pixel_accuracy, iou_score

warnings.filterwarnings('ignore', message='The value of the smallest subnormal for.*float64')


# Combined BCE + Dice loss
class CombinedLoss(nn.Module):
    def __init__(self, bce_weight=0.5):
        super(CombinedLoss, self).__init__()
        self.bce = nn.BCEWithLogitsLoss()
        self.bce_weight = bce_weight

    def forward(self, y_pred, y_true):
        bce_loss = self.bce(y_pred, y_true)
        dice = dice_coefficient(torch.sigmoid(y_pred), y_true)
        dice_loss = 1 - dice
        return self.bce_weight * bce_loss + (1 - self.bce_weight) * dice_loss


def display_results(model, dataset, device, train, sample_count):
    model.eval()
    model = model.to(device).half()  # FP16 for Jetson Nano-style inference
    indices = random.sample(range(len(dataset)), sample_count)
    for idx in indices:
        img = dataset[idx]
        if train:
            img, true_mask = dataset[idx]
        img_input = img.unsqueeze(0).to(device).half()
        with torch.no_grad():
            start = time.time()
            pred_mask = model(img_input)
            inference_time = time.time() - start
            print(f'Inference time: {inference_time:.4f} seconds')
        pred_mask_np = torch.sigmoid(pred_mask).cpu().numpy().squeeze()
        pred_mask_np = (pred_mask_np > 0.5).astype(np.uint8) * 255

        # Post-processing: Morphological opening to remove noise
        kernel = np.ones((3, 3), np.uint8)
        pred_mask_np = cv2.morphologyEx(pred_mask_np, cv2.MORPH_OPEN, kernel)

        img_np = img.permute(1, 2, 0).cpu().numpy()
        plt.figure(figsize=(12, 4))
        plt.subplot(1, 3, 1)
        plt.imshow(img_np)
        plt.title("Image")
        plt.axis("off")

        if train:
            true_mask_np = true_mask.cpu().numpy().squeeze()
            plt.subplot(1, 3, 2)
            plt.imshow(true_mask_np, cmap="gray")
            plt.title("True Mask")
            plt.axis("off")

        plt.subplot(1, 3, 3)
        plt.imshow(pred_mask_np, cmap="gray")
        plt.title("Predicted Mask")
        plt.axis("off")
        plt.tight_layout()
        plt.show()


def save(path: str, values: [(str, [])], plot=False):
    torch.save(model.state_dict(), path + '.pth')
    print(f"Saved model checkpoint at: {path + '.pth'}")
    if plot:
        plot_training(values)


def plot_training(values: [(str, [])]):
    fig = plt.figure(figsize=(10, 6))
    epochs_list = list(range(1, len(values[0][1]) + 1))
    for name, data in values:
        plt.plot(epochs_list, data, label=name)
    plt.grid()
    plt.legend()
    plt.xlabel("Epoch")
    plt.ylabel("Metric")
    plt.savefig(os.path.join(MODELS_DIR, VERSION, f'{MODEL_NAME}_plot.png'), dpi=300, bbox_inches='tight')
    with open(os.path.join(MODELS_DIR, VERSION, f'{MODEL_NAME}_plot.pkl'), 'wb') as f:
        pickle.dump(fig, f)
    plt.show()
    plt.close()


def training(device, transform_img, transform_mask, num_workers):
    global model
    IMAGES_DIR = f"./../MaskGenerator/Dataset/{VERSION}/Images"
    MASKS_DIR = f"./../MaskGenerator/Dataset/{VERSION}/Masks"

    dataset = TrainingDataset(IMAGES_DIR, MASKS_DIR, transform_img=transform_img, transform_mask=transform_mask)
    generator = torch.Generator().manual_seed(25)
    train_dataset, val_dataset = random_split(dataset, [int(len(dataset) * 0.8), int(len(dataset) * 0.2)],
                                              generator=generator)

    train_dataloader = DataLoader(
        dataset=train_dataset,
        num_workers=num_workers,
        pin_memory=True,
        batch_size=BATCH_SIZE,
        shuffle=True
    )
    val_dataloader = DataLoader(
        dataset=val_dataset,
        num_workers=num_workers,
        pin_memory=True,
        batch_size=BATCH_SIZE,
        shuffle=False
    )

    model = EfficientLiteSeg(in_channels=3, out_channels=1).to(device)
    if os.path.isfile(os.path.join(MODELS_DIR, VERSION, f'{MODEL_NAME}.pth')):
        model.load_state_dict(torch.load(os.path.join(MODELS_DIR, VERSION, f'{MODEL_NAME}.pth'), weights_only=True))
        print(f'Loaded model: {os.path.join(MODELS_DIR, VERSION, f"{MODEL_NAME}.pth")}')

    optimizer = optim.AdamW(model.parameters(), lr=LEARNING_RATE, weight_decay=1e-4)
    criterion = CombinedLoss(bce_weight=0.5).to(device)
    scaler = torch.amp.GradScaler('cuda')
    scheduler = torch.optim.lr_scheduler.ReduceLROnPlateau(optimizer, mode='min', factor=0.5, patience=2)

    train_losses, train_dcs, train_ious, train_accs = [], [], [], []
    val_losses, val_dcs, val_ious, val_accs = [], [], [], []
    nb_epoch_no_amelioration = 0
    best_val_loss = float('inf')

    for epoch in tqdm(range(EPOCHS), desc="Epochs"):
        model.train()
        train_running_loss = 0
        train_running_dc = 0
        train_running_iou = 0
        train_running_acc = 0
        steps = 0

        for idx, (img, mask) in enumerate(tqdm(train_dataloader, desc="Training", leave=False, dynamic_ncols=True, mininterval=0.1,smoothing=0.3)):
            img = img.to(device)
            mask = mask.to(device)

            optimizer.zero_grad(set_to_none=True)
            with torch.amp.autocast('cuda', dtype=torch.float16):
                y_pred = model(img)
                loss = criterion(y_pred, mask)
                dc = dice_coefficient(y_pred, mask)
                acc = pixel_accuracy(y_pred, mask)
                iou = iou_score(y_pred, mask)

            scaler.scale(loss).backward()
            torch.nn.utils.clip_grad_norm_(model.parameters(), max_norm=1.0)
            scaler.step(optimizer)
            scaler.update()

            train_running_loss += loss.item()
            train_running_dc += dc.item()
            train_running_iou += iou
            train_running_acc += acc
            steps += 1

            # if idx % 10 == 0:
            #     print(f"GPU Memory: {torch.cuda.memory_allocated() / 1024 ** 2:.2f} MB")

        train_loss = train_running_loss / steps
        train_dc = train_running_dc / steps
        train_iou = train_running_iou / steps
        train_acc = train_running_acc / steps

        train_losses.append(train_loss)
        train_dcs.append(train_dc)
        train_ious.append(train_iou)
        train_accs.append(train_acc)

        model.eval()
        val_running_loss = 0
        val_running_dc = 0
        val_running_iou = 0
        val_running_acc = 0
        val_steps = 0

        with torch.no_grad():
            for idx, (img, mask) in enumerate(tqdm(val_dataloader, desc="Validation", leave=False)):
                img = img.to(device)
                mask = mask.to(device)
                with torch.amp.autocast('cuda', dtype=torch.float16):
                    y_pred = model(img)
                    loss = criterion(y_pred, mask)
                    dc = dice_coefficient(y_pred, mask)
                    acc = pixel_accuracy(y_pred, mask)
                    iou = iou_score(y_pred, mask)

                val_running_loss += loss.item()
                val_running_dc += dc.item()
                val_running_iou += iou
                val_running_acc += acc
                val_steps += 1

        val_loss = val_running_loss / val_steps
        val_dc = val_running_dc / val_steps
        val_iou = val_running_iou / val_steps
        val_acc = val_running_acc / val_steps

        val_losses.append(val_loss)
        val_dcs.append(val_dc)
        val_ious.append(val_iou)
        val_accs.append(val_acc)

        scheduler.step(val_loss)  # Adjust learning rate

        print("-" * 10 + f' EPOCH {epoch + 1} ' + "-" * 10)
        data = [
            ["", "train", "val"],
            ["Loss", f"{train_loss:.4f}", f"{val_loss:.4f}"],
            ["DICE", f"{train_dc:.4f}", f"{val_dc:.4f}"],
            ["IoU", f"{train_iou:.4f}", f"{val_iou:.4f}"],
            ["Pixel Accuracy", f"{train_acc:.4f}", f"{val_acc:.4f}"],
        ]
        print(tabulate(data))
        print("-" * 30)

        if (epoch + 1) % 5 == 0:
            checkpoint_path = os.path.join(MODELS_DIR, VERSION, f"model_epoch_{epoch + 1}")
            save(checkpoint_path, [
                ('train loss', train_losses),
                ('val loss', val_losses),
                ('train dice', train_dcs),
                ('val dice', val_dcs),
                ('train iou', train_ious),
                ('val iou', val_ious),
                ('train accuracy', train_accs),
                ('val accuracy', val_accs)
            ])

        if val_loss < best_val_loss:
            best_val_loss = val_loss
            checkpoint_path = os.path.join(MODELS_DIR, VERSION, f"best_val_loss")
            save(checkpoint_path, [
                ('train loss', train_losses),
                ('val loss', val_losses),
                ('train dice', train_dcs),
                ('val dice', val_dcs),
                ('train iou', train_ious),
                ('val iou', val_ious),
                ('train accuracy', train_accs),
                ('val accuracy', val_accs)
            ])
            nb_epoch_no_amelioration = 0
        else:
            nb_epoch_no_amelioration += 1

        if nb_epoch_no_amelioration >= EARLY_STOPPING_PATIENCE or math.isnan(train_loss) or math.isnan(val_loss):
            print(f'Early stop after {epoch + 1} epochs')
            break

    save(os.path.join(MODELS_DIR, VERSION, 'final_model'), [
        ('train loss', train_losses),
        ('val loss', val_losses),
        ('train dice', train_dcs),
        ('val dice', val_dcs),
        ('train iou', train_ious),
        ('val iou', val_ious),
        ('train accuracy', train_accs),
        ('val accuracy', val_accs)
    ], True)


def testing(device, transform_img, transform_mask):
    TEST_IMAGES_DIR = "./../car_pictures/320_180"
    IMAGES_DIR = f"./../MaskGenerator/Dataset/{VERSION}/Images"
    MASKS_DIR = f"./../MaskGenerator/Dataset/{VERSION}/Masks"

    dataset = TrainingDataset(IMAGES_DIR, MASKS_DIR, transform_img=transform_img, transform_mask=transform_mask)
    dataset_test = TestDataset(TEST_IMAGES_DIR, transform_img=transform_img)

    model = EfficientLiteSeg(in_channels=3, out_channels=1).to(device)
    model_path = os.path.join(MODELS_DIR, VERSION, f'{MODEL_NAME}.pth')
    if os.path.isfile(model_path):
        print(f'Loading model: {model_path}')
        model.load_state_dict(torch.load(model_path, weights_only=True))
        print(f'Loaded model: {model_path}')
    else:
        print(f'Model not found at {model_path}. Using untrained model.')

    display_results(model=model, dataset=dataset, device=device, train=True, sample_count=5)
    display_results(model=model, dataset=dataset_test, device=device, train=False, sample_count=15)


def main():
    global IMG_HEIGHT, IMG_WIDTH, BATCH_SIZE, EPOCHS, LEARNING_RATE, EARLY_STOPPING_PATIENCE
    global VERSION, MODEL_NAME, MODELS_DIR, TRAIN, TEST

    device = torch.device("cuda" if torch.cuda.is_available() else "cpu")
    num_workers = os.cpu_count() // 2  # Use half of CPU cores for PC efficiency

    if not os.path.exists(MODELS_DIR):
        os.makedirs(MODELS_DIR)
    if not os.path.exists(os.path.join(MODELS_DIR, VERSION)):
        os.makedirs(os.path.join(MODELS_DIR, VERSION))

    if TRAIN:
        training(device, transform_img, transform_mask, num_workers)
    if TEST:
        testing(device, transform_img, transform_mask)


def handle_interrupt(signal, frame):
    print("\nTraining interrupted. Saving model...")
    torch.save(model.state_dict(), os.path.join(MODELS_DIR, VERSION, 'final_model.pth'))
    print("Model saved.")
    sys.exit(0)


if __name__ == '__main__':
    IMG_HEIGHT, IMG_WIDTH = 180, 320
    BATCH_SIZE = 16  # Optimized for PC
    EPOCHS = 100
    LEARNING_RATE = 1e-5
    EARLY_STOPPING_PATIENCE = 4
    VERSION = '0.26'
    MODEL_NAME = 'final_model'
    MODELS_DIR = 'models_Unet'
    TRAIN = True
    TEST = True

    transform_img = transforms.Compose([
        transforms.Resize((IMG_HEIGHT, IMG_WIDTH)),
        Pad((0, 6, 0, 6)),
        # transforms.RandomHorizontalFlip(p=0.5),
        # transforms.RandomRotation(10),
        # transforms.ColorJitter(brightness=0.2, contrast=0.2, saturation=0.2),
        transforms.ToTensor(),
    ])
    transform_mask = transforms.Compose([
        transforms.Resize((IMG_HEIGHT, IMG_WIDTH)),
        Pad((0, 6, 0, 6)),
        # transforms.RandomHorizontalFlip(p=0.5),
        # transforms.RandomRotation(10),
        transforms.ToTensor(),
    ])

    signal.signal(signal.SIGINT, handle_interrupt)
    main()
